...
const char * const BITCOIN_CONF_FILENAME = "blocx.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //blocx-tx using stdin
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BLOCXCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BLOCXCore
    // Mac: ~/Library/Application Support/BLOCXCore
    // Unix: ~/.blocxcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BLOCX";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BLOCX";
#else
    // Unix
    return pathRet / ".blocx";
#endif
#endif
}
...
//    std::string strCopyrightHolders = strPrefix + strprintf(" %u-%u ", nStartYear, nEndYear) + strprintf(_(COPYRIGHT_HOLDERS).translated, _(COPYRIGHT_HOLDERS_SUBSTITUTION));
    ...
    // Check for untranslated substitution to make sure BLOCX Core copyright is not removed by accident
    if (copyright_devs.find("BLOCX Core") == std::string::npos) {
